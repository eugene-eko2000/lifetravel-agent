...
def _log_background_task(task: asyncio.Task[None]) -> None:
    ...
    if exc is not None:
        logger.error(
            "Background inventory hotel message task failed",
            exc_info=(type(exc), exc, exc.__traceback__),
        )


async def _process_hotel_incoming(
...
async def run_inventory_hotel_subscriber() -> None:
    ...
    while True:
        try:
                ...
                async with queue.iterator() as queue_iter:
                    async for incoming in queue_iter:
                        task = asyncio.create_task(
                            _process_hotel_incoming(incoming, exchange, cfg, sender)
                        )
            ...
            logger.exception(
                "Inventory hotel subscriber connection or consumer loop failed; reconnecting in %s s",
